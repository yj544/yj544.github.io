...
- JIT Compiler는 단어 그대로 실시간으로 ByteCode를 기계어로 컴파일하는 컴파일러이며 이 과정에서 코드 최적화도 함께 진행한다.
...
- JVM은 실행 단계에서 인터프리터를 통해 즉각 ByteCode를 만들고 이후에 C1 컴파일러를 통해 적당히 최적화된 ByteCode를 생성하다가 C2 컴파일러를 사용한다. 
...
- JVMCI (Java Virtual Machine Compiler Interface) 와 JIT
   ...
   - JVMCI : 컴파일러 인터페이스로 C2 컴파일러가 아닌 다른 컴파일러를 사용할 수 있는 환경을 제공한다.
      ...
      ```
      - Graal 컴파일러는 함수를 컴파일할 때 해당 함수의 ByteCode를 파라미터로 전달하면 컴파일된 기계어를 얻는다. 
